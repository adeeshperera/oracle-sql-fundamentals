...
-- =====================================================
-- Creates a complete business database structure with 
-- departments, employees, products, customers and orders

-- DEPARTMENT TABLE: Stores organizational departments
CREATE TABLE department (
  department_id NUMBER(5),          -- Unique department identifier
  department_name VARCHAR2(100),    -- Department name (e.g., Sales, IT)
  CONSTRAINT pk_dept PRIMARY KEY (department_id)
);

-- EMPLOYEE TABLE: Stores employee information with departmental relationships
CREATE TABLE employee (
  employee_id NUMBER(5),            -- Unique employee identifier
  first_name VARCHAR2(100),         -- Employee first name
  last_name VARCHAR2(100),          -- Employee last name
  salary NUMBER(8),                 -- Annual salary (can be NULL)
  department_id NUMBER(5),          -- Links to department table
  manager_id NUMBER(5),             -- Self-referencing for manager hierarchy
...
-- PRODUCT TABLE: Stores company products by department
CREATE TABLE product (
  product_id NUMBER(5),             -- Unique product identifier
  product_name VARCHAR2(100),       -- Product name
  price NUMBER(10, 2),              -- Product price with 2 decimal places
  department_id NUMBER(5),          -- Department that handles this product
...
CREATE TABLE customer (
  customer_id NUMBER(5),            -- Unique customer identifier
  first_name VARCHAR2(100),         -- Customer first name
  last_name VARCHAR2(100),          -- Customer last name
  address_state VARCHAR2(10),       -- Customer's state/region
  email_address VARCHAR2(350),      -- Customer email (not validated format)
  CONSTRAINT pk_cust PRIMARY KEY (customer_id)
);

-- CUSTOMER_ORDER TABLE: Tracks product orders by customers
CREATE TABLE customer_order (
  order_id NUMBER(5),               -- Unique order identifier
  customer_id NUMBER(5),            -- Links to customer table
  product_id NUMBER(5),             -- Links to product table
...
-- =====================================================
-- Populates all tables with realistic test data for
-- learning and practicing SQL queries

-- DEPARTMENT DATA: 9 different business departments
-- Insert company departments from Executive to Maintenance
...
-- EMPLOYEE DATA: 200 employees with hierarchical manager relationships
-- Includes various salary ranges, departments, and hire dates from 2010-2017
-- Note: Some employees have NULL salaries and manager_id = NULL indicates top-level executives
...
-- PRODUCT DATA: 8 products across Hardware and Software departments
-- Mix of physical items (Monitor, Desk, Chair) and software applications
...
-- CUSTOMER DATA: 10 customers from various US states
-- Note: Some email addresses are incomplete/invalid for testing purposes
...
-- ORDER DATA: 50 customer orders spanning 2015-2017
-- Shows purchase history linking customers to products with order dates
...
-- This makes all INSERT operations permanent
COMMIT;
